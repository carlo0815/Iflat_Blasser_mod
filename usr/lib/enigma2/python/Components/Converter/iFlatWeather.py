...
config.plugins.NfrHD.woeid = ConfigNumber(default = "640161")
config.plugins.NfrHD.tempUnit = ConfigSelection(default = "Celsius", choices = [("Celsius", _("Celsius")),("Fahrenheit", _("Fahrenheit"))])

weather_data = None

class iFlatWeather(Converter, object):
	...
	def getCF(self):
		if config.plugins.NfrHD.tempUnit.value == "Fahrenheit":
			return "°F"
		else: 
			return "°C"
...
class WeatherData:
	def __init__(self):
		...
		if config.plugins.NfrHD.refreshInterval.value > 0:
			self.timer = eTimer()
	...
	def GetWeather(self):
		timeout = config.plugins.NfrHD.refreshInterval.value * 1000 * 60
		if timeout > 0:
			self.timer.start(timeout, True)
			print "NfrHD lookup for ID " + str(config.plugins.NfrHD.woeid.value)
			url = "http://query.yahooapis.com/v1/public/yql?q=select%20item%20from%20weather.forecast%20where%20woeid%3D%22"+str(config.plugins.NfrHD.woeid.value)+"%22&format=xml"
